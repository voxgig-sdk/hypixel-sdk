...
func TestGuildEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HYPIXEL_TEST_GUILD_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func guildBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HYPIXEL_TEST_GUILD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HYPIXEL_TEST_GUILD_ENTID"])

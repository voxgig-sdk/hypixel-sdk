...
import (
	...
	omni "github.com/voxgig-sdk/hypixel-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HypixelError); is {
				out := map[string]any{
					"name":    "HypixelError",
...
func omniCtx(arg any, client *sdk.HypixelSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HypixelSDK); is {
			client = live

...
class SkyBlockEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.SkyBlock(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.SkyBlock(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    sky_block_ref01_data_dt0_load_result = Helpers.to_map(sky_block_ref01_data_dt0_loaded.respond_to?(:data_get) ? sky_block_ref01_data_dt0_loaded.data_get : sky_block_ref01_data_dt0_loaded)

...
class PlayerDataEntity
  ...
  def make
    opts = @_entopts.dup
    PlayerDataEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HypixelHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PlayerData, Hash] the current PlayerData data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HypixelHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PlayerData fields)
  def match_get
  ...
  #
  # @param reqmatch [PlayerDataLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PlayerData.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PlayerData, Hash] the loaded PlayerData; raises HypixelError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = HypixelHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [PlayerDataListMatch, Hash, nil] match filter (any subset of
  #   PlayerData fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<PlayerData>, Array] the matching PlayerData items; raises HypixelError on failure

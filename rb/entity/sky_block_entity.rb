...
class SkyBlockEntity
  ...
  def make
    opts = @_entopts.dup
    SkyBlockEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.SkyBlock.load works with no args).
  ...
  #
  # @param reqmatch [SkyBlockListMatch, Hash, nil] match filter (any subset of
  #   SkyBlock fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control

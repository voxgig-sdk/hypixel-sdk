...
class HypixelTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HypixelTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HypixelTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HypixelTestRunner.getenv("HYPIXEL_TEST_LIVE")
        override = HypixelTestRunner.getenv("HYPIXEL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HypixelTestRunner.getenv(key)
        ...
        explain = HypixelTestRunner.getenv("HYPIXEL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HYPIXEL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HypixelTestRunner._test_control is not None:
            return HypixelTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HypixelTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HypixelTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HypixelTestRunner.load_test_control()
...
def load_env_local():
    HypixelTestRunner.load_env_local()


def env_override(m):
    return HypixelTestRunner.env_override(m)


def entity_data(v):
    return HypixelTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HypixelTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HypixelTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HypixelTestRunner.load_test_control()


def live_client_options():
    return HypixelTestRunner.live_client_options()


def live_delay_ms():
    return HypixelTestRunner.live_delay_ms()

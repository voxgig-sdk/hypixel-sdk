...
from hypixel_sdk.hypixel_types import (
    PlayerData,
    PlayerDataLoadMatch,
    PlayerDataListMatch,
)
...
class PlayerDataEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PlayerDataEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> PlayerData:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PlayerData().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PlayerData]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PlayerData().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",

...
The Lua SDK for the Hypixel API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Guild(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Hypixel(nil):load(
  { id = "test01" }, nil
)
...
The main module (`hypixel_sdk`) exports the SDK constructor

...
describe("SkyBlockDirect", function()
  it("should direct-list-sky_block", function()
    local setup = sky_block_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-sky_block", setup.live and "live" or "unit")
  ...
  it("should direct-load-sky_block", function()
    local setup = sky_block_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-sky_block", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function sky_block_direct_setup(mockres)

...
describe("PlayerDataDirect", function()
  it("should direct-list-player_data", function()
    local setup = player_data_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-player_data", setup.live and "live" or "unit")
  ...
  it("should direct-load-player_data", function()
    local setup = player_data_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-player_data", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function player_data_direct_setup(mockres)

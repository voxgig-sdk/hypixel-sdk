...
local SkyBlockEntity = {}
SkyBlockEntity.__index = SkyBlockEntity


function SkyBlockEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SkyBlockEntity)
  self._name = "sky_block"
  self._client = client
...
function SkyBlockEntity:get_name()
  return self._name
end


function SkyBlockEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SkyBlockEntity.new(self._client, opts)
end
...
function SkyBlockEntity:mark_deleted()
  self._deleted = true
end


function SkyBlockEntity:deleted()
  return true == self._deleted
end


function SkyBlockEntity:data_set(args)
  if args ~= nil then
...
function SkyBlockEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SkyBlockEntity:match_set(args)
  if args ~= nil then
...
function SkyBlockEntity:match_get()
...
function SkyBlockEntity:stream(action, args, callopts)
...
---@param reqmatch SkyBlockLoadMatch
---@param ctrl? table
---@return SkyBlock
---@return string? err
function SkyBlockEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch SkyBlockListMatch
---@param ctrl? table
---@return SkyBlock[]
---@return string? err
function SkyBlockEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function SkyBlockEntity:_run_op(ctx, post_done)
  local utility = self._utility

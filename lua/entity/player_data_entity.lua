...
local PlayerDataEntity = {}
PlayerDataEntity.__index = PlayerDataEntity


function PlayerDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PlayerDataEntity)
  self._name = "player_data"
  self._client = client
...
function PlayerDataEntity:get_name()
  return self._name
end


function PlayerDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PlayerDataEntity.new(self._client, opts)
end
...
function PlayerDataEntity:mark_deleted()
  self._deleted = true
end


function PlayerDataEntity:deleted()
  return true == self._deleted
end


function PlayerDataEntity:data_set(args)
  if args ~= nil then
...
function PlayerDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PlayerDataEntity:match_set(args)
  if args ~= nil then
...
function PlayerDataEntity:match_get()
...
function PlayerDataEntity:stream(action, args, callopts)
...
function PlayerDataEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local HousingEntity = {}
HousingEntity.__index = HousingEntity


function HousingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HousingEntity)
  self._name = "housing"
  self._client = client
...
function HousingEntity:get_name()
  return self._name
end


function HousingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HousingEntity.new(self._client, opts)
end
...
function HousingEntity:mark_deleted()
  self._deleted = true
end


function HousingEntity:deleted()
  return true == self._deleted
end


function HousingEntity:data_set(args)
  if args ~= nil then
...
function HousingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HousingEntity:match_set(args)
  if args ~= nil then
...
function HousingEntity:match_get()
...
function HousingEntity:stream(action, args, callopts)
...
---@param reqmatch HousingLoadMatch
---@param ctrl? table
---@return Housing
---@return string? err
function HousingEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch HousingListMatch
---@param ctrl? table
---@return Housing[]
---@return string? err
function HousingEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function HousingEntity:_run_op(ctx, post_done)
  local utility = self._utility

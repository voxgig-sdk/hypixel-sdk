...
function ResourceEntity.new(client, entopts)
  entopts = entopts or {}
...
function ResourceEntity:get_name()
  return self._name
end


function ResourceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResourceEntity.new(self._client, opts)
end
...
function ResourceEntity:mark_deleted()
  self._deleted = true
end


function ResourceEntity:deleted()
  return true == self._deleted
end


function ResourceEntity:data_set(args)
  if args ~= nil then
...
function ResourceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResourceEntity:match_set(args)
  if args ~= nil then
...
function ResourceEntity:match_get()
...
function ResourceEntity:stream(action, args, callopts)
...
---@param reqmatch ResourceLoadMatch
---@param ctrl? table
---@return Resource
---@return string? err
function ResourceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ResourceEntity:_run_op(ctx, post_done)
  local utility = self._utility

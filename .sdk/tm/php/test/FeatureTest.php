...
class FtCtrl extends HypixelControl
{
...
class FtClient
{
    ...
    private HypixelUtility $utility;

    public function __construct(HypixelUtility $utility, array $options)
    {
    ...
    public function get_utility(): HypixelUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HypixelContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HypixelContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HypixelConfig::make_feature($name);
    ...
    private static function buildUrl(HypixelSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HypixelError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HypixelResponse($fetched);
        }
    ...
    private function fail(HypixelContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HypixelContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
module HypixelUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = HypixelResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HypixelResponse.new(fetched)
    else

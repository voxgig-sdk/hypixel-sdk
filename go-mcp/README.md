...
[MCP](https://modelcontextprotocol.io) server exposing the Hypixel SDK as
two agent tools — `hypixel_list` and `hypixel_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hypixel-mcp)
make build

# 2. Provide credentials via the environment
export HYPIXEL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user hypixel \
  -- /absolute/path/to/hypixel-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hypixel-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// hypixel_list: first page of records
{ "entity": "housing" }
{ "entity": "housing", "query": { } }

// hypixel_load: one record by id
{ "entity": "guild", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hypixel-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hypixel \
     -- "$PWD"/dist/*/hypixel-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hypixel_list` and `hypixel_load` tools now appear
   in new sessions. Ask the agent to *"list housing using hypixel"*
   and it calls `hypixel_list` with `{"entity":"housing"}`.
...
```sh
export HYPIXEL_APIKEY=sk_live_xxx            # API key
export HYPIXEL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `hypixel_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hypixel_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HYPIXEL_APIKEY` | API key sent with every request. |
| `HYPIXEL_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

guild | housing | other | player | player_data | resource | sky_block

### Smoke test via HTTP (raw JSON-RPC)

```sh
./hypixel-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hypixel_load","arguments":{"entity":"guild","query":{"id":1}}}}'
```

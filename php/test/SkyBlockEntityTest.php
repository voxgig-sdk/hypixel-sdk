...
class SkyBlockEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = HypixelSDK::test(null, null);
        $ent = $testsdk->SkyBlock(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->SkyBlock(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->SkyBlock(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = sky_block_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "sky_block." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HYPIXEL_TEST_SKY_BLOCK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $sky_block_ref01_data = null;
        if (count($sky_block_ref01_data_raw) > 0) {
...
function sky_block_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/sky_block/SkyBlockTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["sky_block01", "sky_block02", "sky_block03"] as $k) {
    ...
    $entid_env_raw = getenv("HYPIXEL_TEST_SKY_BLOCK_ENTID");

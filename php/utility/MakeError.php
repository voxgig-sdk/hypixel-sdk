...
// Hypixel SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class HypixelMakeError
{
    public static function call(?HypixelContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HypixelContext([], null);
        }
        $op = $ctx->op ?? new HypixelOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HypixelResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HypixelError) ? $err->msg : (string)$err;
        $msg = "HypixelSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new HypixelError('', $msg, $ctx);

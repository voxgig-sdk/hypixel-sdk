...
//     (\`new HypixelSDK(...)\` / \`HypixelSDK.test(...)\` become
//     \`HypixelSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"guild":{"test01":{"id":"test01"}},"housing":{"test01":{"id":"test01"}},"other":{"test01":{"id":"test01"}},"player":{"test01":{"id":"test01"}},"player_data":{"test01":{"id":"test01"}},"resource":{"test01":{"id":"test01"}},"sky_block":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HypixelSDK(...)\` and any
// \`HypixelSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HypixelSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HypixelSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

...
describe('PlayerDataDirect', async () => {
  ...
  test('direct-load-player_data', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-player_data', setup.live)) return
  ...
  test('direct-list-player_data', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-player_data', setup.live)) return
    const { client, calls } = setup

// Command Hypixel-cli is a CLI + REPL for the Hypixel SDK,
...
const prompt = "hypixel"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "guild housing other player player_data resource sky_block"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HYPIXEL_APIKEY (api key), HYPIXEL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HYPIXEL_APIKEY carries the API key and
	// HYPIXEL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HYPIXEL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HYPIXEL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHypixelSDK(opts)

...
boru-driven command-line client **and** interactive REPL for the Hypixel
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hypixel-cli)
make build
...
./hypixel-cli load 1 guild            # {id:1} shorthand
./hypixel-cli load '{id:1}' guild       # explicit match map

# 5. Override the API base URL for a single call
HYPIXEL_BASE=https://api.example.com ./hypixel-cli load 1 guild

# 6. No arguments -> interactive REPL
./hypixel-cli
hypixel> load 1 guild
hypixel> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hypixel-cli
   ```
...
```sh
./hypixel-cli load 1 guild          # scalar shorthand for {id:1}
./hypixel-cli load '{id:1}' guild     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `hypixel>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `guild`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hypixel-cli`. |
...
(`./hypixel-cli <expr>`) and interactively (the REPL), and expressions compose the
